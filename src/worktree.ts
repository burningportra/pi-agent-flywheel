...
const WORKTREE_DIR = ".pi-agent-flywheel/worktrees";
...
/**
 * Find worktrees in `.pi-agent-flywheel/worktrees/` that aren't in the tracked list.
  ...
  for (const orphan of orphans) {
    ...
    if (orphan.isDirty) {
      const commitResult = await autoCommitWorktree(
        pi,
        orphan.path,
        `[pi-agent-flywheel] auto-commit: recovery of orphaned worktree`
      );
    ...
    for (const idx of indices) {
      ...
      try {
        const commitResult = await autoCommitWorktree(
          this.pi,
          info.path,
          `[pi-agent-flywheel] auto-commit: safe cleanup of worktree step-${idx}`
        );
        if (commitResult.ok && commitResult.data) {
          summary.autoCommitted++;
        }
